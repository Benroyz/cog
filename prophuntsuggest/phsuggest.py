...
class PHSuggestionBox(commands.Cog):
    """custom cog for a configurable phsuggestion box"""
    ...
    @commands.group(name="setphsuggest", pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_guild=True)
    async def setphsuggest(self, ctx):
        """configuration settings"""

        pass

    
    @setphsuggest.command(name="on", pass_context=True, no_pm=True)
    async def _setphsuggest_on(self, ctx):
    ...
    @setphsuggest.command(name="off", pass_context=True, no_pm=True)
    async def _setphsuggest_off(self, ctx):
    ...
    @setphsuggest.command(name="block", pass_context=True, no_pm=True)
    async def block(self, ctx, user: discord.Member):
        """Blocks a user from making phsuggestions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id in blocked_ids:
                await ctx.send("This user is already in the block list, did you mean to `--setphsuggest unblock`?")
            else:
                blocked_ids.append(user.id)
                await ctx.send("User blocked.")

    @setphsuggest.command(name="unblock", pass_context=True, no_pm=True)
    async def unblock(self, ctx, user: discord.Member):
        """Unblocks a user from making phsuggestions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id not in blocked_ids:
                await ctx.send("This user isn't in the block list, did you mean to `--setphsuggest block`?")
            else:
                blocked_ids.remove(user.id)
                await ctx.send("User unblocked.")

    @setphsuggest.command(name="anonymous", pass_context=True, no_pm=True)
    async def anonymous(self, ctx):
        """Toggles whether or not the phsuggestions are anonymous."""
        guild = ctx.guild
    ...
    @commands.command(name="phsuggest", pass_context=True)
    async def makephsuggestion(self, ctx):
        "make a phsuggestion by following the prompts"
        ...
        async with group.blocked_ids() as blocked_ids:
            ...
            if message is None:
                ...
                await self.send_phsuggest(message, guild)
                await author.send("Your phsuggestion was submitted.")

    async def send_phsuggest(self, message, guild):
        author = guild.get_member(message.author.id)
        group = self.config.guild(guild)
        phsuggestion = message.clean_content
        avatar = author.avatar_url if author.avatar \
            else author.default_avatar_url

        em = discord.Embed(description=phsuggestion,

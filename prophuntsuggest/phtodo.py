...
class PHToDo(commands.Cog):
    """custom cog for a configurable phtodoion box"""
    ...
    @commands.group(name="setphtodo", pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_guild=True)
    async def setphtodo(self, ctx):
        """configuration settings"""

        pass

    
    @setphtodo.command(name="on", pass_context=True, no_pm=True)
    async def _setphtodo_on(self, ctx):
    ...
    @setphtodo.command(name="off", pass_context=True, no_pm=True)
    async def _setphtodo_off(self, ctx):
    ...
    @setphtodo.command(name="block", pass_context=True, no_pm=True)
    async def block(self, ctx, user: discord.Member):
        """Blocks a user from making phtodoions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id in blocked_ids:
                await ctx.send("This user is already in the block list, did you mean to `--setphtodo unblock`?")
            else:
                blocked_ids.append(user.id)
                await ctx.send("User blocked.")

    @setphtodo.command(name="unblock", pass_context=True, no_pm=True)
    async def unblock(self, ctx, user: discord.Member):
        """Unblocks a user from making phtodoions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id not in blocked_ids:
                await ctx.send("This user isn't in the block list, did you mean to `--setphtodo block`?")
            else:
                blocked_ids.remove(user.id)
                await ctx.send("User unblocked.")

    @setphtodo.command(name="anonymous", pass_context=True, no_pm=True)
    async def anonymous(self, ctx):
        """Toggles whether or not the phtodoions are anonymous."""
        guild = ctx.guild
    ...
    async def makephtodoion(self, ctx):
        ...
        async with group.blocked_ids() as blocked_ids:
            ...
            if message is None:
                ...
                await self.send_phtodo(message, guild)
                await author.send("Your phtodoion was submitted.")

    async def send_phtodo(self, message, guild):
        author = guild.get_member(message.author.id)
        group = self.config.guild(guild)
        phtodoion = message.clean_content
        avatar = author.avatar_url if author.avatar \
            else author.default_avatar_url

        em = discord.Embed(description=phtodoion,

...
class TTTUKtodo(commands.Cog):
    ...
    @commands.group(name="settttuktodo", pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_guild=True)
    async def settttuktodo(self, ctx):
        """configuration settings"""

        pass

    
    @settttuktodo.command(name="on", pass_context=True, no_pm=True)
    async def _settttuktodo_on(self, ctx):
        ...
        async with guild_group.channels_enabled() as channels_enabled:
            ...
            if channel.id in channels_enabled:
                await ctx.send("To Do is already on in this channel.")
            else:
                channels_enabled.append(channel.id)

                await ctx.send("To Do is now on in this channel.")

    @settttuktodo.command(name="off", pass_context=True, no_pm=True)
    async def _settttuktodo_off(self, ctx):
    ...
    @settttuktodo.command(name="block", pass_context=True, no_pm=True)
    async def block(self, ctx, user: discord.Member):
        """Blocks a user from making tttuktodoions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id in blocked_ids:
                await ctx.send("This user is already in the block list, did you mean to `--settttuktodo unblock`?")
            else:
                blocked_ids.append(user.id)
                await ctx.send("User blocked.")

    @settttuktodo.command(name="unblock", pass_context=True, no_pm=True)
    async def unblock(self, ctx, user: discord.Member):
        """Unblocks a user from making tttuktodoions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id not in blocked_ids:
                await ctx.send("This user isn't in the block list, did you mean to `--settttuktodo block`?")
            else:
                blocked_ids.remove(user.id)
                await ctx.send("User unblocked.")

    @settttuktodo.command(name="anonymous", pass_context=True, no_pm=True)
    async def anonymous(self, ctx):
        """Toggles whether or not the tttuktodoions are anonymous."""
        guild = ctx.guild
    ...
    @commands.command(name="tttuktodo", pass_context=True)
    @checks.admin_or_permissions(manage_messages=True)
    async def maketttuktodoion(self, ctx):
        "make a tttuktodoion by following the prompts"
        ...
        async with group.blocked_ids() as blocked_ids:
            ...
            if message is None:
                ...
                await self.send_tttuktodo(message, guild)
                await author.send("Your tttuktodoion was submitted.")

    async def send_tttuktodo(self, message, guild):
        author = guild.get_member(message.author.id)
        group = self.config.guild(guild)
        tttuktodoion = message.clean_content
        avatar = author.avatar_url if author.avatar \
            else author.default_avatar_url

        em = discord.Embed(description=tttuktodoion,

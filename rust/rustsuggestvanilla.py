...
class RustSuggest(commands.Cog):
    """custom cog for a configurable suggestmoddedion box"""
    ...
    @commands.group(name="setsuggestmodded", pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_guild=True)
    async def setsuggestmodded(self, ctx):
        """configuration settings"""

        pass

    
    @setsuggestmodded.command(name="on", pass_context=True, no_pm=True)
    async def _setsuggestmodded_on(self, ctx):
        """Turn on suggestmoddedionBox in the current channel"""
        guild_group = self.config.guild(ctx.guild)

        async with guild_group.channels_enabled() as channels_enabled:
            channel = ctx.message.channel
            if channel.id in channels_enabled:
                await ctx.send("suggestmoddedionBox is already on in this channel.")
            else:
                channels_enabled.append(channel.id)

                await ctx.send("suggestmoddedionBox is now on in this channel.")

    @setsuggestmodded.command(name="off", pass_context=True, no_pm=True)
    async def _setsuggestmodded_off(self, ctx):
        """Turn off suggestmoddedionBox in the current channel"""
        guild_group = self.config.guild(ctx.guild)

        async with guild_group.channels_enabled() as channels_enabled:
            channel = ctx.message.channel
            if channel.id not in channels_enabled:
                await ctx.send("suggestmoddedionBox is already off in this channel.")
            else:
                channels_enabled.remove(channel.id)

                await ctx.send("suggestmoddedionBox is now off in this channel.")

    @setsuggestmodded.command(name="block", pass_context=True, no_pm=True)
    async def block(self, ctx, user: discord.Member):
        """Blocks a user from making suggestmoddedions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id in blocked_ids:
                await ctx.send("This user is already in the block list, did you mean to `--setsuggestmodded unblock`?")
            else:
                blocked_ids.append(user.id)
                await ctx.send("User blocked.")

    @setsuggestmodded.command(name="unblock", pass_context=True, no_pm=True)
    async def unblock(self, ctx, user: discord.Member):
        """Unblocks a user from making suggestmoddedions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id not in blocked_ids:
                await ctx.send("This user isn't in the block list, did you mean to `--setsuggestmodded block`?")
            else:
                blocked_ids.remove(user.id)
                await ctx.send("User unblocked.")

    @setsuggestmodded.command(name="anonymous", pass_context=True, no_pm=True)
    async def anonymous(self, ctx):
        """Toggles whether or not the suggestmoddedions are anonymous."""
        guild = ctx.guild
        ...
        if current_val:
            await ctx.send("suggestmoddedions are now anonymous.")
        else:
            await ctx.send("suggestmoddedions are no longer anonymous.")

        await self.config.guild(guild).anonymous.set(current_val)

    @commands.command(name="suggestmodded", pass_context=True)
    async def makesuggestmoddedion(self, ctx):
        "make a suggestmoddedion by following the prompts"
        ...
        async with group.blocked_ids() as blocked_ids:
            ...
            if message is None:
                ...
                await self.send_suggestmodded(message, guild)
                await author.send("Your suggestion was submitted.")

    async def send_suggestmodded(self, message, guild):
        author = guild.get_member(message.author.id)
        group = self.config.guild(guild)
        suggestmoddedion = message.clean_content
        avatar = author.avatar_url if author.avatar \
            else author.default_avatar_url

        em = discord.Embed(description=suggestmoddedion,

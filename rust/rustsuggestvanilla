...
class RustSuggestVanilla(commands.Cog):
    """custom cog for a configurable suggestvanillaion box"""
    ...
    @commands.group(name="setsuggestvanilla", pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_guild=True)
    async def setsuggestvanilla(self, ctx):
        """configuration settings"""

        pass

    
    @setsuggestvanilla.command(name="on", pass_context=True, no_pm=True)
    async def _setsuggestvanilla_on(self, ctx):
        """Turn on suggestvanillaionBox in the current channel"""
        guild_group = self.config.guild(ctx.guild)

        async with guild_group.channels_enabled() as channels_enabled:
            channel = ctx.message.channel
            if channel.id in channels_enabled:
                await ctx.send("suggestvanillaionBox is already on in this channel.")
            else:
                channels_enabled.append(channel.id)

                await ctx.send("suggestvanillaionBox is now on in this channel.")

    @setsuggestvanilla.command(name="off", pass_context=True, no_pm=True)
    async def _setsuggestvanilla_off(self, ctx):
        """Turn off suggestvanillaionBox in the current channel"""
        guild_group = self.config.guild(ctx.guild)

        async with guild_group.channels_enabled() as channels_enabled:
            channel = ctx.message.channel
            if channel.id not in channels_enabled:
                await ctx.send("suggestvanillaionBox is already off in this channel.")
            else:
                channels_enabled.remove(channel.id)

                await ctx.send("suggestvanillaionBox is now off in this channel.")

    @setsuggestvanilla.command(name="block", pass_context=True, no_pm=True)
    async def block(self, ctx, user: discord.Member):
        """Blocks a user from making suggestvanillaions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id in blocked_ids:
                await ctx.send("This user is already in the block list, did you mean to `--setsuggestvanilla unblock`?")
            else:
                blocked_ids.append(user.id)
                await ctx.send("User blocked.")

    @setsuggestvanilla.command(name="unblock", pass_context=True, no_pm=True)
    async def unblock(self, ctx, user: discord.Member):
        """Unblocks a user from making suggestvanillaions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id not in blocked_ids:
                await ctx.send("This user isn't in the block list, did you mean to `--setsuggestvanilla block`?")
            else:
                blocked_ids.remove(user.id)
                await ctx.send("User unblocked.")

    @setsuggestvanilla.command(name="anonymous", pass_context=True, no_pm=True)
    async def anonymous(self, ctx):
        """Toggles whether or not the suggestvanillaions are anonymous."""
        guild = ctx.guild
        ...
        if current_val:
            await ctx.send("suggestvanillaions are now anonymous.")
        else:
            await ctx.send("suggestvanillaions are no longer anonymous.")

        await self.config.guild(guild).anonymous.set(current_val)

    @commands.command(name="suggestvanilla", pass_context=True)
    async def makesuggestvanillaion(self, ctx):
        "make a suggestvanillaion by following the prompts"
        ...
        async with group.blocked_ids() as blocked_ids:
            ...
            if message is None:
                ...
                await self.send_suggestvanilla(message, guild)
                await author.send("Your suggestion was submitted.")

    async def send_suggestvanilla(self, message, guild):
        author = guild.get_member(message.author.id)
        group = self.config.guild(guild)
        suggestvanillaion = message.clean_content
        avatar = author.avatar_url if author.avatar \
            else author.default_avatar_url



        em = discord.Embed(description=suggestvanillaion,

...
class SuggestzsBox(commands.Cog):
    """custom cog for a configurable suggestzsion box"""
    ...
    @commands.group(name="setsuggestzs", pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_guild=True)
    async def setsuggestzs(self, ctx):
        """configuration settings"""

        pass

    
    @setsuggestzs.command(name="on", pass_context=True, no_pm=True)
    async def _setsuggestzs_on(self, ctx):
        """Turn on suggestzsionBox in the current channel"""
        guild_group = self.config.guild(ctx.guild)

        async with guild_group.channels_enabled() as channels_enabled:
            channel = ctx.message.channel
            if channel.id in channels_enabled:
                await ctx.send("suggestzsionBox is already on in this channel.")
            else:
                channels_enabled.append(channel.id)

                await ctx.send("suggestzsionBox is now on in this channel.")

    @setsuggestzs.command(name="off", pass_context=True, no_pm=True)
    async def _setsuggestzs_off(self, ctx):
        """Turn off suggestzsionBox in the current channel"""
        guild_group = self.config.guild(ctx.guild)

        async with guild_group.channels_enabled() as channels_enabled:
            channel = ctx.message.channel
            if channel.id not in channels_enabled:
                await ctx.send("suggestzsionBox is already off in this channel.")
            else:
                channels_enabled.remove(channel.id)

                await ctx.send("suggestzsionBox is now off in this channel.")

    @setsuggestzs.command(name="block", pass_context=True, no_pm=True)
    async def block(self, ctx, user: discord.Member):
        """Blocks a user from making suggestzsions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id in blocked_ids:
                await ctx.send("This user is already in the block list, did you mean to `--setsuggestzs unblock`?")
            else:
                blocked_ids.append(user.id)
                await ctx.send("User blocked.")

    @setsuggestzs.command(name="unblock", pass_context=True, no_pm=True)
    async def unblock(self, ctx, user: discord.Member):
        """Unblocks a user from making suggestzsions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id not in blocked_ids:
                await ctx.send("This user isn't in the block list, did you mean to `--setsuggestzs block`?")
            else:
                blocked_ids.remove(user.id)
                await ctx.send("User unblocked.")

    @setsuggestzs.command(name="anonymous", pass_context=True, no_pm=True)
    async def anonymous(self, ctx):
        """Toggles whether or not the suggestzsions are anonymous."""
        guild = ctx.guild
        ...
        if current_val:
            await ctx.send("suggestzsions are now anonymous.")
        else:
            await ctx.send("suggestzsions are no longer anonymous.")

        await self.config.guild(guild).anonymous.set(current_val)

    @commands.command(name="suggestzs", pass_context=True)
    async def makesuggestzsion(self, ctx):
        "make a suggestzsion by following the prompts"
        ...
        async with group.blocked_ids() as blocked_ids:
            ...
            if message is None:
                ...
                await self.send_suggestzs(message, guild)
                await author.send("Your suggestion was submitted.")

    async def send_suggestzs(self, message, guild):
        author = guild.get_member(message.author.id)
        group = self.config.guild(guild)
        suggestzsion = message.clean_content
        avatar = author.avatar_url if author.avatar \
            else author.default_avatar_url

        em = discord.Embed(description=suggestzsion,

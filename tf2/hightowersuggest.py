...
class HighTowerSuggest(commands.Cog):
    """custom cog for a configurable High Tower Suggestion box"""
    ...
    @commands.group(name="sethightowersuggest", pass_context=True, no_pm=True)
    @checks.admin_or_permissions(manage_guild=True)
    async def sethightowersuggest(self, ctx):
        """configuration settings"""

        pass

    
    @sethightowersuggest.command(name="on", pass_context=True, no_pm=True)
    async def _sethightowersuggest_on(self, ctx):
        """Turn on hightowersuggestionBox in the current channel"""
        guild_group = self.config.guild(ctx.guild)

        async with guild_group.channels_enabled() as channels_enabled:
            channel = ctx.message.channel
            if channel.id in channels_enabled:
                await ctx.send("hightowersuggestionBox is already on in this channel.")
            else:
                channels_enabled.append(channel.id)

                await ctx.send("hightowersuggestionBox is now on in this channel.")

    @sethightowersuggest.command(name="off", pass_context=True, no_pm=True)
    async def _sethightowersuggest_off(self, ctx):
        """Turn off hightowersuggestionBox in the current channel"""
        guild_group = self.config.guild(ctx.guild)

        async with guild_group.channels_enabled() as channels_enabled:
            channel = ctx.message.channel
            if channel.id not in channels_enabled:
                await ctx.send("hightowersuggestionBox is already off in this channel.")
            else:
                channels_enabled.remove(channel.id)

                await ctx.send("hightowersuggestionBox is now off in this channel.")

    @sethightowersuggest.command(name="block", pass_context=True, no_pm=True)
    async def block(self, ctx, user: discord.Member):
        """Blocks a user from making hightowersuggestions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id in blocked_ids:
                await ctx.send("This user is already in the block list, did you mean to `--sethightowersuggest unblock`?")
            else:
                blocked_ids.append(user.id)
                await ctx.send("User blocked.")

    @sethightowersuggest.command(name="unblock", pass_context=True, no_pm=True)
    async def unblock(self, ctx, user: discord.Member):
        """Unblocks a user from making hightowersuggestions."""
        guild = ctx.guild
        group = self.config.guild(guild)

        async with group.blocked_ids() as blocked_ids:
            if user.id not in blocked_ids:
                await ctx.send("This user isn't in the block list, did you mean to `--sethightowersuggest block`?")
            else:
                blocked_ids.remove(user.id)
                await ctx.send("User unblocked.")

    @sethightowersuggest.command(name="anonymous", pass_context=True, no_pm=True)
    async def anonymous(self, ctx):
        """Toggles whether or not the hightowersuggestions are anonymous."""
        guild = ctx.guild
        ...
        if current_val:
            await ctx.send("hightowersuggestions are now anonymous.")
        else:
            await ctx.send("hightowersuggestions are no longer anonymous.")
    ...
    async def makehightowersuggestion(self, ctx):
        ...
        async with group.blocked_ids() as blocked_ids:
            ...
            if message is None:
                ...
                await self.send_hightowersuggest(message, guild)
                await author.send("Your suggestion was submitted.")

    async def send_hightowersuggest(self, message, guild):
        author = guild.get_member(message.author.id)
        group = self.config.guild(guild)
        hightowersuggestion = message.clean_content
        avatar = author.avatar_url if author.avatar \
            else author.default_avatar_url

        em = discord.Embed(description=hightowersuggestion,
